...
const seriesList = [
  {
    ...
    promise: "看完后能用大白话讲清大模型是怎么切分文字、怎样消耗显存、为什么能被提示词引导，以及为什么流畅的回答可以一本正经地瞎编。",
    chapters: [
      {
        id: "foundation",
        title: "大模型的物理底座",
  ...
  },
  {
    ...
    title: "大模型是怎么炼成的",
    subtitle: "从接龙到深度思考",
    description: "从预训练基座、合成数据、SFT、偏好对齐到推理模型与瘦身，拆解一个大模型诞生的工业工序。",
    promise: "看完后能分清 Base、Chat/Instruct、RLHF/DPO 与慢思考推理模型的本质区别与工业工序。",
    chapters: [
      {
        id: "raw-model",
  ...
  },
  {
    ...
    title: "给大模型装上手和脚",
    subtitle: "连接资料与外部世界",
    description: "从 RAG、向量检索、工具调用、结构化输出到 MCP 与 Skill，看模型如何接上私有资料和外部动作。",
    promise: "看完后能讲清 RAG 怎样补私有事实、Tool Calling 怎样触发外部动作，以及 MCP 和 Skill 怎样标准化能力。",
    chapters: [
      {
        id: "retrieve",
  ...
  },
  {
    ...
    title: "AI 智能体怎么替人干活",
    subtitle: "从问答走向自主行动",
    description: "从 ReAct 循环、宿主底盘、记忆、规划，到多智能体、电脑操作与安全评测，看智能体如何从问答变成干活。",
    promise: "看完后能分清 Chatbot 与 Autonomous Agent，搞懂 Harness、工具循环、自检和安全落地。",
    chapters: [
      {
        id: "loop",
  ...
  },
  {
    ...
    promise: "看完后能讲清跨模态如何变成 Token、本地运行靠什么，以及堆算力也跨不过去的限制。",
    chapters: [
      {
        id: "senses",
...
for (const series of seriesList) {
  const indexFile = path.join(rootDir, "docs", "series", series.id, "index.md");
  const indexContent = `---
title: "第${seasonLabel(series.season)}季 · ${series.title}"
...
<LearningMap series-id="${series.id}" />
`;
  fs.writeFileSync(indexFile, indexContent, "utf8");

  for (const chapter of series.chapters) {
    for (const article of chapter.articles) {
      const artFile = path.join(rootDir, "docs", "series", series.id, `${article.id}.md`);
      if (!fs.existsSync(artFile)) {
        continue;
      }
      const existing = fs.readFileSync(artFile, "utf8");

      if (article.id === "large-model" || article.id === "transformer") {
        let updated = existing
          .replace(/^title:.*$/m, `title: "${article.title}"`)
          .replace(/^description:.*$/m, `description: "${article.description}"`)
          .replace(/^# .*$/m, `# ${article.title}`);
        fs.writeFileSync(artFile, updated, "utf8");
      } else {
        const prereqs = article.prerequisites.length === 0
          ? " []"
          : "\n" + article.prerequisites.map(p => `  - "${p}"`).join("\n");
        const points = article.points.map(p => `- ${p}`).join("\n");
        const content = `---
title: "${article.title}"
...
`;
        fs.writeFileSync(artFile, content, "utf8");
      }
    }
  }
}
console.log("All markdown files synchronized successfully.");
